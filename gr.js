// Goods Receipt Notes Data and Table Rendering
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Metrics from GRNs-Template.csv
    const grnMetrics = {
        totalGRNs: 3,
        totalValue: 8058975,
        inProcessGRNs: 1,
        partiallyReceivedGRNs: 1,
        fullyReceivedGRNs: 1
    };
    
    // Update metric cards with data from GRNs-Template.csv
    function updateMetricCards() {
        const grnTotalElement = document.getElementById('grn-total-count');
        const grnTotalValueElement = document.getElementById('grn-total-value');
        const grnInProcessElement = document.getElementById('grn-in-process-count');
        const grnPartiallyReceivedElement = document.getElementById('grn-partially-received');
        const grnFullyReceivedElement = document.getElementById('grn-fully-received');
        
        if (grnTotalElement) grnTotalElement.textContent = grnMetrics.totalGRNs;
        if (grnTotalValueElement) grnTotalValueElement.textContent = grnMetrics.totalValue.toLocaleString();
        if (grnInProcessElement) grnInProcessElement.textContent = grnMetrics.inProcessGRNs;
        if (grnPartiallyReceivedElement) grnPartiallyReceivedElement.textContent = grnMetrics.partiallyReceivedGRNs;
        if (grnFullyReceivedElement) grnFullyReceivedElement.textContent = grnMetrics.fullyReceivedGRNs;
    }
    
    // Data from GRNs-Template.csv
    const grnData = [
        {
            "Title": "Summer Campaign Tier 1",
            "Value": "8042775",
            "Aging": "24",
            "Raised by": "Mussaifah Mana Alotaiba",
            "Pending with": "HONG XING WU ENTERTAINMENT & MEDIA INVESTMENT (H.K.) CO., LIMITED"
        },
        {
    ...
    function applyTableStyling() {
        ...
        const table = document.querySelector('#grn-table');
        if (!table) return;
    ...
    // Function to render the GRN table with pagination
    function renderGRNTable(sortedData = null) {
        const tableContainer = document.querySelector('#grn-table').closest('.card-body');
        ...
        const dataToUse = sortedData || grnData;
